...
### 本步目标
- 以最小工作量把后端风险结果展示到现有前端中。
- 复用侧边栏结构，先形成“风险排行 + 单事件详情”的最小闭环。
...
### 本步新增或变更的能力
- 新增 `frontend/src/components/RiskPanel.vue`
  - 请求 `/risk/ranking`
  - 支持时间窗口切换
  - 支持最小风险等级筛选
  - 展示时间、区域、震级、风险等级、风险分数
  - 点击列表项后请求 `/risk/events/<event_unid>`
  - 通过抽屉展示评估摘要和特征摘要
- 在 `frontend/src/components/Sidebar.vue` 中接入 `RiskPanel`
  - 保持现有侧边栏结构
  - 不改动主地图和既有地震列表主逻辑
...
- 执行 `pnpm build`
  - 结果：前端构建通过
  - 结果：`RiskPanel.vue` 成功参与打包，没有出现模板或导入错误
- 构建产物中前端主包成功生成
  - 结果：`dist/index.html`、CSS 和 JS 产物已输出
  - 结果：仅出现打包体积告警，没有阻塞性错误

### 本步结论
- 第十步已经达到可验收状态。
- 当前前端已经具备直接展示风险排行和查看单事件评估信息的最小能力。
- 当前仍未进入全链路联调回归阶段。

### 给后续开发者的提示
- 下一步应严格进入“MVP 联调与回归测试”，重点检查新增风险面板是否影响现有地图、热力图、时间轴和 GIS 分析开关。
- 如果后续要继续增强前端，不要先重做整体布局，优先补联调验证和已有能力的稳定性。

## 第十一步完成记录：MVP 联调与回归测试

### 本步目标
- 从近空数据库开始，按顺序验证初始化、历史导入、特征刷新、风险评分、风险接口和前端构建。
- 确认新增风险链路没有破坏已有核心查询和地图相关主链路。
- 补充简短接口说明和新增数据字典。
...
### 本步新增或变更的能力
- 在 `运行说明.md` 中补充了 MVP 运行顺序：
  - 数据库初始化
  - 历史数据导入
  - 事件特征刷新
  - 风险评分生成
  - 前后端启动
- 在 `运行说明.md` 中补充了当前关键接口说明：
  - 历史统计接口
  - 风险排行接口
  - 单事件风险详情接口
- 在 `运行说明.md` 中补充了新增数据字典：
  - `earthquake_features`
  - `earthquake_risk_scores`

### 本步自验证
- 使用临时数据库执行完整 MVP 后端链路
  - 结果：`import_history.py` 成功导入 13 条历史记录
  - 结果：`refresh_features.py` 成功处理 13 条候选事件并写入 13 条特征
  - 结果：`score_risk.py` 成功处理 13 条候选特征并写入 13 条评分
- 使用 Flask 测试客户端回归旧接口与新接口
  - `GET /earthquakes`
    - 结果：返回 `200`
  - `GET /stats/cluster`
    - 结果：返回 `200`
  - `GET /stats/history/timeline?years=3&bucket=month`
    - 结果：返回 `200`
  - `GET /stats/history/region_dist?years=3&limit=5`
    - 结果：返回 `200`
  - `GET /risk/ranking?hours=30000&limit=5&min_risk_level=low`
    - 结果：返回 `200`，且有 5 条排行结果
  - `GET /risk/events/<event_unid>`
    - 结果：返回 `200`，返回结构包含 `event`、`feature_summary`、`risk`
- 执行 `pnpm build`
  - 结果：前端生产构建通过
  - 结果：仅存在现有包体积告警，没有阻塞性错误

### 本步结论
- 第十一步已经达到可验收状态。
- MVP 核心闭环已经具备可复现、可联调、可演示的最小状态。
- 当前未发现新增风险链路对已有核心接口造成阻塞性回归。
